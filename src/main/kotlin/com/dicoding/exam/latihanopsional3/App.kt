...
fun manipulateString(str: String, int: Int): String {
    ...
    return if (hasDigit) {
        val firstDigitIndex = str.indexOfFirst { it.isDigit() }
        val numberPart = str.substring(firstDigitIndex)
        val cleanedNumber = numberPart.replaceFirst("^0+".toRegex(), "")
        val result = cleanedNumber.toInt() * int
        str.substring(0, firstDigitIndex) + result
    } else {
        str + int
    }
}
